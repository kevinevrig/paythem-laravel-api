...
class PTNAPI extends Controller
{
    /**
     ...
     */
    public function __construct(string $environment = '', int $appID = 2848, string $iv = '')
    {
        $this->environment                  = $environment;
        $this->serverURI                    = "https://vvs{$this->environment}.paythem.net/API/{$appID}/";
        $this->innerVars['DEBUG_OUTPUT']    = $this->innerVars['FAULTY_PROXY'] = $this->innerVars['SERVER_DEBUG'] = false;
    ...
    public function callAPI(bool $debug = false)
    {
        ...
        $this->result = $this->innerVars['ENCRYPT_RESPONSE'] ? $this->doDecrypt($this->response['CONTENT']) : $this->response['CONTENT'];
